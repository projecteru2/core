...
// WatchLambda provides a mock function with given fields: ctx
func (_m *Store) WatchLambda(ctx context.Context) <-chan *types.LambdaStatus {
	ret := _m.Called(ctx)

	var r0 <-chan *types.LambdaStatus
	if rf, ok := ret.Get(0).(func(context.Context) <-chan *types.LambdaStatus); ok {
		r0 = rf(ctx)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(<-chan *types.LambdaStatus)
		}
	}
