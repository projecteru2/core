...
func (m *Mercury) LoadNodeCert(ctx context.Context, node *types.Node) error {
	ca, cert, key, err := m.loadCert(ctx, node.Name)
	if err != nil {
		return err
	}
...
func (m *Mercury) makeClient(ctx context.Context, node *types.Node) (engine.API, error) {
	...
	return enginefactory.GetEngine(ctx, m.config, node.Name, node.Endpoint, ca, cert, key)
}
	...
	for i, format := range []string{nodeCaKey, nodeCertKey, nodeKeyKey} {
		...
		if err != nil {
			if !errors.Is(err, types.ErrInvaildCount) {
				log.WithFunc("store.etcdv3.loadCert").Error(ctx, err, "get key")
				return "", "", "", err
			}
			continue
		}
