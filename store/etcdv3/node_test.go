...
func TestAddNode(t *testing.T) {
	...
	// wrong endpoint
	_, err = m.AddNode(ctx, nodename, "abc", podname, "", "", "", cpu, share, memory, storage, labels, nil, nil, nil)
	assert.Error(t, err)
	...
	// wrong no pod
	_, err = m.AddNode(ctx, nodename, endpoint, "abc", "", "", "", cpu, share, memory, storage, labels, nil, nil, nil)
	assert.Error(t, err)
	// AddNode
