...
func TestNode(t *testing.T) {
	...
	// UpdateNodeResource
	assert.NoError(t, m.UpdateNodeResource(ctx, node, nil, int64(100), "-"))
	...
	assert.NoError(t, m.UpdateNodeResource(ctx, node, types.CPUMap{"0": 10}, int64(0), "-"))
	newNode, err = m.GetNodeByName(ctx, nodename)
	assert.NoError(t, err)
	...
	err = m.UpdateNodeResource(ctx, node, nil, int64(0), "abc")
	assert.Error(t, err)
