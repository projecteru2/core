...
// SetContainersStatus provides a mock function with given fields: ctx, status, ttls, force
func (_m *Cluster) SetContainersStatus(ctx context.Context, status map[string]types.StatusMeta, ttls map[string]int64, force bool) ([]types.StatusMeta, error) {
	ret := _m.Called(ctx, status, ttls, force)

	var r0 []types.StatusMeta
	if rf, ok := ret.Get(0).(func(context.Context, map[string]types.StatusMeta, map[string]int64, bool) []types.StatusMeta); ok {
		r0 = rf(ctx, status, ttls, force)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]types.StatusMeta)
		}
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, map[string]types.StatusMeta, map[string]int64, bool) error); ok {
		r1 = rf(ctx, status, ttls, force)
	} else {
		r1 = ret.Error(1)
	}
