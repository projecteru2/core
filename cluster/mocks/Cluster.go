...
// ReapLambda provides a mock function with given fields: ctx, exit
func (_m *Cluster) ReapLambda(ctx context.Context, exit <-chan struct{}) (<-chan struct{}, error) {
	ret := _m.Called(ctx, exit)

	var r0 <-chan struct{}
	if rf, ok := ret.Get(0).(func(context.Context, <-chan struct{}) <-chan struct{}); ok {
		r0 = rf(ctx, exit)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, <-chan struct{}) error); ok {
		r1 = rf(ctx, exit)
	} else {
		r1 = ret.Error(1)
	}
