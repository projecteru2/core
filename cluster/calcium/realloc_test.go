...
func TestRealloc(t *testing.T) {
	...
	ch, err := c.ReallocResource(ctx, []string{"c1"}, -1, 2*int64(units.GiB), types.TriKeep, types.TriKeep, nil)
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, 2*int64(units.GiB), types.TriKeep, types.TriKeep, nil)
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, 2*int64(units.MiB), types.TriKeep, types.TriKeep, nil)
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1", "c2"}, 0.1, 2*int64(units.MiB), types.TriKeep, types.TriKeep, nil)
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, int64(units.MiB), types.TriKeep, types.TriKeep, types.MustToVolumeBindings([]string{"AUTO:/data:rw:50"}))
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, int64(units.MiB), types.TriKeep, types.TriKeep, types.MustToVolumeBindings([]string{"AUTO:/data:rw:1"}))
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1", "c2"}, 0.1, int64(units.MiB), types.TriKeep, types.TriKeep, types.MustToVolumeBindings([]string{"AUTO:/data:rw:1"}))
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c3", "c4"}, 0.1, 2*int64(units.MiB), types.TriKeep, types.TriKeep, types.MustToVolumeBindings([]string{"AUTO:/data0:rw:-50"}))
	assert.NoError(t, err)
	for r := range ch {
		assert.Error(t, r.Error)
	}
...
func TestReallocBindCpu(t *testing.T) {
	...
	ch, err := c.ReallocResource(ctx, []string{"c5"}, 0.1, 2*int64(units.MiB), types.TriFalse, types.TriKeep, nil)
	for r := range ch {
	...
	ch, err = c.ReallocResource(ctx, []string{"c6"}, 0.1, 2*int64(units.MiB), types.TriTrue, types.TriKeep, nil)
	for r := range ch {
		assert.NoError(t, r.Error)
	}
	assert.NoError(t, err)
	...
	ch, err = c.ReallocResource(ctx, []string{"c6", "c5"}, -0.1, 2*int64(units.MiB), types.TriTrue, types.TriKeep, nil)
	for r := range ch {
		assert.NoError(t, r.Error)
	}
	assert.NoError(t, err)
	...
	ch, err = c.ReallocResource(ctx, []string{"c6", "c5"}, -0.1, 2*int64(units.MiB), types.TriFalse, types.TriKeep, nil)
	for r := range ch {
		assert.NoError(t, r.Error)
	}
	assert.NoError(t, err)
