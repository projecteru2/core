...
func TestRealloc(t *testing.T) {
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, 2*types.GByte)
	assert.NoError(t, err)
	for r := range ch {
		assert.False(t, r.Success)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1"}, 0.1, 2*types.MByte)
	assert.NoError(t, err)
	for r := range ch {
		assert.False(t, r.Success)
	}
	...
	ch, err = c.ReallocResource(ctx, []string{"c1", "c2"}, 0.1, 2*types.MByte)
	assert.NoError(t, err)
	for r := range ch {
		assert.False(t, r.Success)
	}
	...
	assert.Equal(t, node1.MemCap, types.GByte)
	...
	ch, err = c.ReallocResource(ctx, []string{"c3", "c4"}, 0.1, 2*types.MByte)
	assert.NoError(t, err)
	for r := range ch {
	...
	assert.Equal(t, node2.MemCap, types.GByte-4*types.MByte)
}
