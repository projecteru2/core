...
func TestRemoveImage(t *testing.T) {
	...
	_, err = c.RemoveImage(ctx, "podname", nil, []string{}, 0, false)
	assert.Error(t, err)
	...
	ch, err := c.RemoveImage(ctx, "podname", nil, []string{"xx"}, 0, false)
	for c := range ch {
	...
	ch, err = c.RemoveImage(ctx, "podname", nil, []string{"xx"}, 0, true)
	for c := range ch {
...
func TestCacheImage(t *testing.T) {
	...
	// fail by get nodes
	store.On("GetNodesByPod", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, types.ErrBadCount).Once()
	_, err = c.CacheImage(ctx, "podname", nil, []string{}, 0)
	assert.Error(t, err)
	store.On("GetNodesByPod", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return([]*types.Node{}, nil).Once()
	// fail 0 nodes
	_, err = c.CacheImage(ctx, "podname", nil, []string{}, 0)
	...
	ch, err := c.CacheImage(ctx, "podname", nil, []string{"xx"}, 0)
	for c := range ch {
		assert.False(t, c.Success)
	}
