			...
			go func(ID string) {
				...
				if err := c.withContainerLocked(ctx, ID, func(container *types.Container) error {
					return c.withNodeLocked(ctx, container.Nodename, func(node *types.Node) (err error) {
						...
						if err = c.store.UpdateNodeResource(ctx, node, container.CPU, container.Quota, container.Memory, container.Storage, container.VolumeMap, store.ActionIncr); err == nil {
							success = true
						}
						return err
					})
