// Code generated by mockery v2.14.0. DO NOT EDIT.

package mocks

import (
	context "context"

	mock "github.com/stretchr/testify/mock"
	metadata "google.golang.org/grpc/metadata"

	pb "github.com/projecteru2/core/rpc/gen"
)

// CoreRPC_RunAndWaitServer is an autogenerated mock type for the CoreRPC_RunAndWaitServer type
type CoreRPC_RunAndWaitServer struct {
	mock.Mock
}

// Context provides a mock function with given fields:
func (_m *CoreRPC_RunAndWaitServer) Context() context.Context {
	ret := _m.Called()

	var r0 context.Context
	if rf, ok := ret.Get(0).(func() context.Context); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(context.Context)
		}
	}

	return r0
}

// Recv provides a mock function with given fields:
func (_m *CoreRPC_RunAndWaitServer) Recv() (*pb.RunAndWaitOptions, error) {
	ret := _m.Called()

	var r0 *pb.RunAndWaitOptions
	if rf, ok := ret.Get(0).(func() *pb.RunAndWaitOptions); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*pb.RunAndWaitOptions)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func() error); ok {
		r1 = rf()
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// RecvMsg provides a mock function with given fields: m
func (_m *CoreRPC_RunAndWaitServer) RecvMsg(m interface{}) error {
	ret := _m.Called(m)

	var r0 error
	if rf, ok := ret.Get(0).(func(interface{}) error); ok {
		r0 = rf(m)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Send provides a mock function with given fields: _a0
func (_m *CoreRPC_RunAndWaitServer) Send(_a0 *pb.AttachWorkloadMessage) error {
	ret := _m.Called(_a0)

	var r0 error
	if rf, ok := ret.Get(0).(func(*pb.AttachWorkloadMessage) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// SendHeader provides a mock function with given fields: _a0
func (_m *CoreRPC_RunAndWaitServer) SendHeader(_a0 metadata.MD) error {
	ret := _m.Called(_a0)

	var r0 error
	if rf, ok := ret.Get(0).(func(metadata.MD) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// SendMsg provides a mock function with given fields: m
func (_m *CoreRPC_RunAndWaitServer) SendMsg(m interface{}) error {
	ret := _m.Called(m)

	var r0 error
	if rf, ok := ret.Get(0).(func(interface{}) error); ok {
		r0 = rf(m)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// SetHeader provides a mock function with given fields: _a0
func (_m *CoreRPC_RunAndWaitServer) SetHeader(_a0 metadata.MD) error {
	ret := _m.Called(_a0)

	var r0 error
	if rf, ok := ret.Get(0).(func(metadata.MD) error); ok {
		r0 = rf(_a0)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// SetTrailer provides a mock function with given fields: _a0
func (_m *CoreRPC_RunAndWaitServer) SetTrailer(_a0 metadata.MD) {
	_m.Called(_a0)
}

type mockConstructorTestingTNewCoreRPC_RunAndWaitServer interface {
	mock.TestingT
	Cleanup(func())
}

// NewCoreRPC_RunAndWaitServer creates a new instance of CoreRPC_RunAndWaitServer. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewCoreRPC_RunAndWaitServer(t mockConstructorTestingTNewCoreRPC_RunAndWaitServer) *CoreRPC_RunAndWaitServer {
	mock := &CoreRPC_RunAndWaitServer{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
