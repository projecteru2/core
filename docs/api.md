...
Image references are built as `hub/namespace/appname:tag` from `registry.hub` and
`registry.namespace`; with no tags, `latest` is used. Registry credentials come from
`registry.auths`.
